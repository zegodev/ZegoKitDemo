...
public class ChatRoom extends Activity {
	...
	private int getRoomNum(int roomKey){
		if(mBCustomRoom){
			...
			return roomKey -  MainActivity.DEMO_ROOMKEY_BASE + 1;
		}
	}
				...
				switch(event)
				{
					case EVENT_LOGIN:
					{
						...
						if(nResult == AVRoomCallback.ERROR_CODE_LOGIN_OK)
						{
							...
							ShowLog("进入房间" + getRoomNum(nRoomKey) +"成功");
							SetRoomTitle("房间:" + getRoomNum(nRoomKey));
						...
						{
							ShowLog("进入房间" + getRoomNum(nRoomKey) +"失败");
						}
					}
					break;
